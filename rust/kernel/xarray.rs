...
impl<T: ForeignOwnable> XArray<T> {
    ...
    fn iter(&self) -> impl Iterator<Item = NonNull<T::PointedTo>> + '_ {
        ...
        // SAFETY: `self.xa` is always valid by the type invariant.
        iter::once(unsafe {
            bindings::xa_find(self.xa.get(), &mut index, u64::MAX, bindings::XA_PRESENT)
        })
        .chain(iter::from_fn(move || {
            // SAFETY: `self.xa` is always valid by the type invariant.
            Some(unsafe {
                bindings::xa_find_after(self.xa.get(), &mut index, u64::MAX, bindings::XA_PRESENT)
            })
        }))
...
impl<'a, T: ForeignOwnable> Guard<'a, T> {
    fn load<F, U>(&self, index: u64, f: F) -> Option<U>
    where
    ...
    pub fn get(&self, index: u64) -> Option<T::Borrowed<'_>> {
        self.load(index, |ptr| {
            // SAFETY: `ptr` came from `T::into_foreign`.
    ...
    ///
    /// Returns the entry at the given index.
    pub fn get_mut(&mut self, index: u64) -> Option<T::BorrowedMut<'_>> {
        self.load(index, |ptr| {
            // SAFETY: `ptr` came from `T::into_foreign`.
