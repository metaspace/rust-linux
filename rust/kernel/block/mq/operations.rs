...
pub trait Operations: Sized {
    ...
    fn init_hctx(hctx_idx: u32) -> Result;
...
impl<T: Operations> OperationsVTable<T> {
    ...
    ///
    /// This function may only be called by blk-mq C infrastructure. `hctx` must
    ///  be a pointer to a valid `blk_mq_hw_ctx`. This function may only be
    ///  called onece before `exit_hctx_callback` is called for the same
    ///  context.
