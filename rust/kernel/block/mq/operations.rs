...
pub trait Operations: Sized {
    ...
    fn poll(_hw_data: <Self::HwData as ForeignOwnable>::Borrowed<'_>) -> bool {
        crate::build_error("TODO: change this to VTABLE_BUILD_ERROR in 6.8")
    }
...
impl<T: Operations> OperationsVTable<T> {
    ...
    unsafe extern "C" fn queue_rq_callback(
        ...
        // function. By C API contract we have exclusive access to `(*bd).rq`
        // for the duration of this function.
        let rq = unsafe { Request::from_ptr_mut((*bd).rq) };

        // TODO:
        // This safety section and the others here are rather long and
        // mostly repeat themselves. Is it possible to put this in its own
        // module and explain the safety invariants in that module and then in
        // these safety sections just refer to some labels from that section?
        ...
        // `ForeignOwnable::into_pointer()` to create `driver_data`. Further,
        ...
        // the `blk-mq` C API, `queue_rq_callback()` will not be called after that
        // point.
    ...
    unsafe extern "C" fn map_queues_callback(tag_set_ptr: *mut bindings::blk_mq_tag_set) {
        let tag_set = unsafe { TagSet::from_ptr(tag_set_ptr) };
