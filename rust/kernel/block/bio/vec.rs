...
impl Segment<'_> {
    ...
    pub fn copy_to_folio(&self, dst_folio: &mut UniqueFolio) -> Result {
        ...
        let mut src_page = ManuallyDrop::new(unsafe { Pages::<0>::from_raw(self.bio_vec.bv_page) });

        src_page.with_page_mapped_local(|src| {
            dst_folio.with_page_mapped_local_mut(0, |dst| {
    ...
    pub fn copy_to_page_atomic(&self, page: &mut Pages<0>) -> Result {
        ...
        // SAFETY: By C API contract `self.offset()` + `self.len()` <=
    ...
    pub fn copy_from_folio(&mut self, src_folio: &UniqueFolio) -> Result {
        ...
        dst_page.with_page_mapped_local_mut(|dst| {
            src_folio.with_page_mapped_local(0, |src| {
                dst.copy_from_slice(src)
            })
        })??;
...
impl<'a> BioSegmentIterator<'a> {
    ...
    unsafe fn len(&self) -> u32 {
        // SAFETY: By safety requirement of this function `self.iter.bi_size` is
        // greater than 0.
        unsafe { self.mp_len().min(crate::PAGE_SIZE - self.offset()) }
    }
    ...
    unsafe fn offset(&self) -> u32 {
        // SAFETY: By safety requirement of this function `self.iter.bi_size` is
        // greater than 0.
        unsafe { self.mp_offset() % crate::PAGE_SIZE }
    }
    ...
    unsafe fn mp_page_idx(&self) -> usize {
        // SAFETY: By safety requirement of this function `self.iter.bi_size` is
        // greater than 0.
        (unsafe { self.mp_offset() } / crate::PAGE_SIZE) as usize
    }
