...
///
/// Target must be `Sync` because timer callbacks effectively could happen in
/// another thread of execution.
pub unsafe trait RawTimer: Sync {
...
unsafe impl<T: Sync> RawTimer for Pin<&T>
where
    //T: TimerCallback,
    T: HasTimer<T>,
{
    fn schedule(self, expires: u64) {
        ...
        let c_timer_ptr = timer_ptr as *const bindings::hrtimer;
...
unsafe impl<'a, T: Sync> RawTimerCallback for Pin<&'a T>
where
    T: HasTimer<T>,
