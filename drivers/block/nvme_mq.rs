...
impl mq::Operations for IoQueueOperations {
    ...
    fn map_queues(tag_set: &mq::TagSetRef) {
        ...
        unsafe {
            ...
            for i in 0..num_maps {
                ...
                let map = &mut (&mut (*tag_set.ptr()).map)[i as usize];
    ...
    T: mq::Operations<RequestData = NvmeRequest>,
{
    match rq.command() {
        ...
        bindings::req_op_REQ_OP_WRITE | bindings::req_op_REQ_OP_READ => {
            ...
            // TODO: Handle unwrap
            let offset = rq.bio().unwrap().iter().bi_sector;
    ...
    T: mq::Operations<RequestData = NvmeRequest>,
{
    ...
    if status != 0 {
        ...
        mq::Request::<T>::request_from_pdu(pdu).end_err(EIO);
        return;
    }

    mq::Request::<T>::request_from_pdu(pdu).end_ok();
}
