...
struct NvmeQueues {
    admin: Option<Ref<nvme_queue::NvmeQueue<nvme_mq::AdminQueueOperations>>>,
    io: Vec<Ref<nvme_queue::NvmeQueue<nvme_mq::IoQueueOperations>>>,
}
...
impl NvmeDevice {
    fn alloc_ns(
        ...
        nsid: u32,
        id: &NvmeIdNs,
        tagset: Ref<mq::TagSet<nvme_mq::IoQueueOperations>>,
        rt: &NvmeLbaRangeType,
    ...
    fn setup_io_queues(
        ...
        admin_queue: &Ref<nvme_queue::NvmeQueue<nvme_mq::AdminQueueOperations>>,
        mq: &mq::RequestQueue<nvme_mq::AdminQueueOperations>,
    ) -> Result<Ref<mq::TagSet<nvme_mq::IoQueueOperations>>> {
        ...
        let tagset = mq::TagSet::try_new(nr_io_queues, dev.clone(), q_depth, 3)?; //TODO: 1 or 3 on demand, depending on polling enabled
    ...
    fn wait_ready(dev: &Ref<DeviceData>) {
    ...
    fn wait_idle(dev: &Ref<DeviceData>) {
    ...
    fn configure_admin_queue(
        ...
        //TODO: Depth?
        let queue_depth = 64;
        let admin_tagset: Ref<mq::TagSet<nvme_mq::AdminQueueOperations>> =
            mq::TagSet::try_new(1, dev.clone(), queue_depth, 1)?;
        let admin_queue: Ref<nvme_queue::NvmeQueue<nvme_mq::AdminQueueOperations>> =
...
static NEXT_ID: kernel::sync::smutex::Mutex<u32> = kernel::sync::smutex::Mutex::new(0);
...
impl pci::Driver for NvmeDevice {
    ...
    fn probe(dev: &mut pci::Device, _id: Option<&Self::IdInfo>) -> Result<Ref<DeviceData>> {
        ...
        let io_queue_count = kernel::num_possible_cpus();
        //let poll_queue_count = io_queue_count - 1;
        let poll_queue_count = io_queue_count;
        //let irq_queue_count = io_queue_count - poll_queue_count;
        let irq_queue_count = io_queue_count;
        ...
        let mut data = Pin::from(kernel::new_device_data!(
            ...
            NvmeData {
                ...
                pci_dev: unsafe { pci::Device::from_ptr(dev.as_ptr()) },
                instance: id,
                shadow: None,
