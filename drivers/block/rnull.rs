...
struct TreeContainer {
    // Even though `XArray` applies locking internaly, we do not want consumers
    // to spin on the internal xarray lock. That lock (xa_lock) is located on
    // the same cache line as the xarray data pointer (xa_head). The effect of
    // this arrangement is that under heavy contention, we often get a cache
    // miss when we try to follow the data pointer after acquiring the lock. We
    // would rather have consumers spinning on another lock, so we do not get a
    // miss on xa_head.
    #[pin]
